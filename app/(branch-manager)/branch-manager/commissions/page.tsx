...
export default function BranchManagerCommissionsPage() {
  ...
  // Get this branch manager's branch_id
  ...
  useEffect(() => {
    async function fetchSalesPeople() {
      ...
      const { data: rawPeople } = await supabase
        ...
        .order('full_name')
      const people = (rawPeople ?? []) as { id: string; full_name: string | null }[]
      setSalesPeople(people.map((p) => ({ id: p.id, name: p.full_name ?? 'Unknown' })))
    }
    fetchSalesPeople()
  }, [])
  ...
  const pendingSessionIds = sessionCommissions.filter((c) => c.status === 'pending').map((c) => c.id)
  const pendingSalesIds   = salesCommissions.filter((c) => c.status === 'pending').map((c) => c.id)

  return (
    <div className="space-y-6">
      <PageHeader
        title="Commissions"
        description="Review and approve trainer and staff commissions for your branch."
      />
