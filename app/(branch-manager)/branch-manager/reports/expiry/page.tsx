...
async function getBrandId(): Promise<string> {
  ...
  if (!brandId) {
    ...
    const { data } = await supabase.from('profiles').select('brand_id').eq('id', user.id).maybeSingle()
    const row = data as { brand_id: string | null } | null
    if (!row?.brand_id) throw new Error('No brand context')
    return row.brand_id
  }
  return brandId
}

export default function BranchManagerExpiryReportPage() {
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <p className="text-sm">
              {new Date(r.gym_access_expires_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </p>
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div>
            <p className="text-sm">
              {new Date(r.pt_sessions_expires_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </p>
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" className="h-7 text-xs" asChild>
            <Link href={`/branch-manager/members/${row.original.member_id}`}>View</Link>
          </Button>
          <Button
            variant="ghost"
            size="sm"
